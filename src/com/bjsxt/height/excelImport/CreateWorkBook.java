...
class ForkJoinReadsheet extends RecursiveTask<String> {
	static XSSFRow  ros;
	private int row;
	...
	protected String compute() {
		...
		XSSFRow tableHead =spreadsheet.getRow(0);*/
		String str = "空";
		int res =0;
		if(row < threshold) {
			...
			Sheet spreadsheet1 = workbookTemp.createSheet();*/
			...
			left.fork();
			right.fork();
			str += "left"+ left.join() +"right"+right.join();
		}
		return str;
	}
	private XSSFWorkbook subSheet(int srcStartRow,int srcEndRow,final XSSFSheet sheet) {
		...
		for (int i = srcStartRow; i < srcEndRow; i++) {
			//新建一行
			XSSFRow targerRow = (XSSFRow) targetSheet.createRow(rowid++);
			...
			while (cellIterator.hasNext()) {
				...
				default: targercell.setCellValue("");
					break;
				}
			}
		}
		//sheet.copyRows(srcStartRow, srcEndRow, destStartRow, cellCopyPolicy);
		/*row = spreadsheet.createRow(rowid++);
		Object[] objectArr = empinfo.get(key);
		int cellid = 0;
		for (Object obj : objectArr) {
	...
	public static void main(String[] args) throws Exception {
		FileInputStream fis = new FileInputStream(new File("typesofcells.xlsx"));
		XSSFWorkbook workbook = new XSSFWorkbook(fis);
		ForkJoinPool forkJoinPool = new ForkJoinPool();// 创建他的线程池
		ForkJoinTask<String> f = forkJoinPool.submit(new ForkJoinReadsheet(5, workbook, true));
